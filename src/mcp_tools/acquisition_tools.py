...
def list_available_signals() -> str:
    """
        ...
        """
    # Failures propagate as exceptions (clean MCP protocol errors) — never
    # hand-built JSON strings with unescaped interpolation.
...
def read_signal_file(filename: str) -> str:
    """
        ...
        """
    # Failures raise (clean MCP protocol errors) — success output is built
    # with json.dumps, never with f-string interpolation into JSON text.
    file_path = safe_resolve(DATA_DIR, filename)

    if not file_path.exists():
        raise FileNotFoundError(
            f"Signal file not found: {filename} — use signal://list or the "
            f"list_signals() tool to see the available files."
        )
    ...
    if signal_array is None:
        raise ValueError(
            f"Could not load signal from {filename} — unsupported format or "
            f"read error. Supported extensions: {sorted(SUPPORTED_EXTENSIONS)}."
        )
...
def read_manual_resource(filename: str) -> str:
    """
        ...
        **IMPORTANT - LLM Usage Guidelines:**
        ...
        """
    # Failures raise (clean MCP protocol errors) — never error dicts
    # returned as success.
    manual_path = safe_resolve(RESOURCES_DIR / "machine_manuals", filename)

    if not manual_path.exists():
        available = [
            f.name for f in (RESOURCES_DIR / "machine_manuals").glob("*.pdf")
        ] + [f.name for f in (RESOURCES_DIR / "machine_manuals").glob("*.txt")]
        raise FileNotFoundError(
            f"Manual not found: {filename} — available manuals: "
...
def list_signals() -> str:
    """
        ...
        """
    # An empty catalog is a legitimate negative outcome (plain message);
    # actual failures raise and surface as MCP protocol errors.
    if not DATA_DIR.exists():
...
def register(mcp: FastMCP) -> None:
    ...
    mcp.resource("signal://list")(list_available_signals)
    mcp.resource("signal://read/{filename}")(read_signal_file)
    mcp.resource("manual://list")(list_manuals_resource)
    mcp.resource("manual://read/{filename}")(read_manual_resource)
    mcp.tool()(list_signals)
    mcp.tool()(generate_test_signal)
    mcp.tool()(load_signal)
    mcp.tool()(list_stored_signals)
    mcp.tool()(get_signal_info)
    mcp.tool()(clear_signal)
    mcp.tool()(clear_all_signals)
